(uiop:define-package :rooms-client/stdio-transport
  (:use :cl
  ...
  (:import-from :jsonrpc/request-response
                :parse-message)
  (:import-from :rooms-client/async-process-stream
                :make-input-stream)
  (:export :stdio-transport))
(in-package :rooms-client/stdio-transport)
...
(defmethod initialize-instance ((instance stdio-transport) &rest initargs)
    ...
    (setf (transport-threads transport)
          (list
           (bt2:make-thread
            ...
            :name "rooms-client processing")
           (bt2:make-thread
            (lambda ()
              (run-reading-loop transport connection))
            :name "rooms-client reading")))
    connection))
